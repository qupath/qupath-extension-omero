...
public class JsonApi2 {
    ...
    public JsonApi2(URI webServerUri, RequestSender requestSender, Credentials credentials) throws URISyntaxException, ExecutionException, InterruptedException {
        ...
        if (credentials.getUserType().equals(Credentials.UserType.REGULAR_USER)) {
            LoginResponse loginResponse = login(this.requestSender, credentials, this.urls, this.serverID, token);
    ...
    private static LoginResponse login(
        ...
        char[] encodedPassword = ApiUtilities.urlEncode(credentials.getPassword().toCharArray());

        byte[] body = ApiUtilities.concatAndConvertToBytes(
                String.join("&", "server=" + serverID, "username=" + credentials.getUsername(), "password=").toCharArray(),
                encodedPassword
        );
